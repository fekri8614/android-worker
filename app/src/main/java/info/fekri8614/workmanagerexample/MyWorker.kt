package info.fekri8614.workmanagerexample
...
class MyWorker(context: Context, workerParameter: WorkerParameters) :
    ...
    companion object {
        ...
        val oneTimeWorkRequest = OneTimeWorkRequestBuilder<MyWorker>().setConstraints(constraints)
            // backoff criteria for retrying the worker
            .setBackoffCriteria(
                BackoffPolicy.EXPONENTIAL,
                // minimum 10 sec.s required
                10, TimeUnit.SECONDS
            ).addTag(TAG)
        ...
        fun executePeriodicWorker(context: Context) {
            WorkManager.getInstance(context).enqueueUniquePeriodicWork(
                    TAG, ExistingPeriodicWorkPolicy.KEEP, periodicWorkRequest
                )
        }
    }

    override fun doWork(): Result {
        // get input data passed
