...
import info.fekri8614.workmanagerexample.ui.theme.WorkManagerExampleTheme
    ...
    private fun listenWorkerUpdate() {
        lifecycleScope.launch {
            WorkManager.getInstance(this@MainActivity)
                .getWorkInfoByIdFlow(MyWorker.oneTimeWorkRequest.id).collect { info ->
            ...
            WorkManager.getInstance(this@MainActivity)
                .getWorkInfoByIdFlow(MyWorker.periodicWorkRequest.id).collect { info ->
                    if (info?.state == WorkInfo.State.RUNNING) {
                        Toast.makeText(this@MainActivity, "Work is Running", Toast.LENGTH_SHORT)
                            .show()
                    } else if (info?.state == WorkInfo.State.SUCCEEDED) {
